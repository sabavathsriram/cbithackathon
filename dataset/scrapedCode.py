...
app = FastAPI(title="Disease Prediction and Recommendation System")

# Load Jinja2 templates (like Flask's render_template)
...
# -----------------------------
# Dictionaries for symptoms & diseases
# -----------------------------
...
def information(predicted_dis):
    disease_desciption = " ".join(description[description['Disease'] == predicted_dis]['Description'])
    disease_precautions = precautions[precautions['Disease'] == predicted_dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.tolist()
    disease_medications = medications[medications['Disease'] == predicted_dis]['Medication'].values.tolist()
    disease_diet = diets[diets['Disease'] == predicted_dis]['Diet'].values.tolist()
    disease_workout = workout[workout['disease'] == predicted_dis]['workout'].values.tolist()
...
def predict(request: Request, symptoms: str = Form(...)):
    if symptoms.strip().lower() == "symptoms":
    ...
    dis_des, precautions, medications, rec_diet, workout_list = information(predicted_disease)

    my_precautions = precautions[0] if precautions else []
    medications = ast.literal_eval(medications[0]) if medications else []
    rec_diet = ast.literal_eval(rec_diet[0]) if rec_diet else []
...
async def api_predict(symptoms: str):
    patient_symptoms = [s.strip().lower() for s in symptoms.split(",")]
    corrected_symptoms = [correct_spelling(s) for s in patient_symptoms if correct_spelling(s)]
    ...
    response = {
        ...
        "medications": ast.literal_eval(medications[0]) if medications else [],
        "diet": ast.literal_eval(rec_diet[0]) if rec_diet else [],
        "workout": workout_list
    }
